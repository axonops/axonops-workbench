...
if hasattr(options, "username") and hasattr(options, "password"):
    ...
    try:
        ...
        # Decrypt Username
        decryptedUsername = b64decode(options.username)
        decryptedUsername = cipher.decrypt(decryptedUsername, "")
        options.username = decryptedUsername.decode("utf-8")
        # Decrypt Password
        decryptedPassword = b64decode(options.password)
        decryptedPassword = cipher.decrypt(decryptedPassword, "")
        options.password = decryptedPassword.decode("utf-8")
...
overrideHost = None
overridePort = None
if hasattr(options, "overrideHost"):
    overrideHost = options.overrideHost
if hasattr(options, "overridePort"):
    overridePort = options.overridePort

varsManifest = None
varsValues = None
if hasattr(options, "varsManifest"):
    varsManifest = options.varsManifest
if hasattr(options, "varsValues"):
    varsValues = options.varsValues
...
if hasattr(options, "ip"):
    DEFAULT_HOST = options.ip
...
class Shell(cmd.Cmd):
    ...
    def __init__(self, hostname, port, color=False,
        ...
        if use_conn:
            ...
            self.conn = Cluster(contact_points=(self.hostname,), port=self.port, cql_version=cqlver,
                                ...
                                ssl_options=sslhandling.ssl_settings(hostname, CONFIG_FILE, varsManifest=varsManifest, varsValues=varsValues, workspaceID=workspaceID) if ssl else None,
    ...
    def perform_statement(self, statement):
        ...
        if future:
            ...
            if self.tracing_enabled:
                try:
                    for trace in future.get_all_query_traces(max_wait_per=self.max_trace_wait,
    ...
    def perform_simple_statement(self, statement):
        ...
        if statement.query_string[:6].lower() == 'select':
        ...
        elif statement.query_string.lower().startswith("list users") or statement.query_string.lower().startswith(
                "list roles"):
            ...
            # CAS INSERT/UPDATE
            self.writeresult("")
            self.print_static_result(result, self.parse_for_update_meta(statement.query_string), with_header=True,
                                     tty=self.tty)
        self.flush_output()
        return True, future
    ...
    def do_expand(self, parsed):
        ...
        """
        self.expand_enabled = SwitchCommand("EXPAND", "Expanded output").execute(self.expand_enabled, parsed,
                                                                                 self.printerr)
    ...
    def do_consistency(self, parsed):
        ...
        if level is None:
            print(
                'Current consistency level is %s.' % (cassandra.ConsistencyLevel.value_to_name[self.consistency_level]))
            return
...
def read_options(cmdlineargs, environment):
    ...
    optvalues.username = givenUsername
    optvalues.password = givenPassword
    ...
    optvalues.connect_timeout = option_with_default(configs.getint, 'connection', 'timeout',
                                                    DEFAULT_CONNECT_TIMEOUT_SECONDS)
    optvalues.request_timeout = option_with_default(configs.getint, 'connection', 'request_timeout',
