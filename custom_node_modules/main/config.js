...
let getConfig = (callback) => {
  try {
    ...
    log.debug('Retrieving the configuration from file...', {'path': AppConfigPath})
    ...
    log.error('Error reading configuration', {'error': e})
...
let setConfig = (configObject, callback = null) => {
  // Add log about this process
  log.info('Writing the app configuration to file...', {'path': AppConfigPath})

  // Attempt to set/write the given configuration object in the app's config file
  try {
    configObject.write(AppConfigPath)
  } catch (e) {
    log.warning('Error writing configuration file', {'error': e})
  }
...
let mergeConfigFiles = (oldConfigFilePath, newConfigFilePath, callback) => {
  try {
    ...
    log.warning('Error in process configuration', {'error': e})
  } finally {
    // Call the callback function
    callback()
  }
