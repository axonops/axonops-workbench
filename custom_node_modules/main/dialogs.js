...
let createDialog = async (window, data) => {
  ...
  Dialog[data.type == undefined ? 'showOpenDialog' : data.type](window, data)
    .then((result) => window.webContents.send(`dialog:${data.id}`, result.filePaths || result.filePath))
    .catch((e) => { log.warning('Error in process dialog', {'error': e})})
}
