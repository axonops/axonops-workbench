...
class Pty {
      ...
      try {
        ...
        try {
          ...
          if (isNaN(parseInt(e.toString()))) {
            log.warning('Error in process terminal', {'error': e})
          }
        }
  ...
  command(command, blockID, currentBuffer, process = 'processMain') {
    ...
    if (OS.platform() != 'darwin') {
      try {
        ...
        log.warning('Error sending command to the pty instance', {'process': process, 'command': command, 'error:': e})
      }
    }
  ...
  close() {
    ...
    log.info('Closing PTY...')
    try {
      this.processMain.destroy()
      Kill(this.processMain.pid, 'SIGKILL')
    } catch (e) {
      log.warning('Something went wrong while closing PTY instance', {'error': e})
    }
  ...
  pause() {
    log.info('Pausing PTY instance socket...')
    this.processMain.pause()
  }

  // Resume the socket
  resume() {
    log.info('Resuming PTY instance socket...')
  ...
  async createCQLSHInstance(data) {
    log.info('Creating cqlsh instance...')
    // Define the Apache Cassandra's version
    let version = this.cassandraVersion
  ...
  getMetadata(metadataPrintRequestID, currentBuffer) {
    ...
    watchFileExistance(tempFile, async (found) => {
      ...
      try {
        ...
        // If the error is a number then don't log the error
        if (isNaN(parseInt(e.toString()))) {
          log.warning('Error deleting temporary file', {'path': tempFile, 'error': e})
        }
      }
    })
  }
  ...
  try {
    ...
    let checkOutput = (output) => {
      ...
      let sendResult = () => {
        ...
        log.info('Test result with cluster/node', {'result': JSON.stringify(result)})
    ...
    IPCMain.on(`process:terminate:${processID}`, (_, __) => {
      log.info('Terminating process...', {'pid': processID})
      ...
      try {
        ...
        log.warning('Something went wrong terminating process', {'pid': processID, 'error': e})
      } finally {
        // Send final status
...
let bashSession = (window, data) => {
  ...
  // When receiving a commad to be executed from the renderer thread
  data.IPCMain.on(`pty:${data.id}:command:bash-session`, (_, command) => {
    try {
      ...
      try {
        ...
        log.warning('Error sending command to PTY instance', {'error': e})
      } catch (e) {}
    }
  })
  ...
  data.IPCMain.on(`pty:${data.id}:close:bash-session`, (_, __) => {
    log.info('Closing bash session...', {'pid': process.pid})
    try {
      // Call the `destroy` function from the pty instance
      process.destroy()
      ...
      log.warning('Error closing bash session', {'pid': process.pid, 'error': e})
    }
  })
}
