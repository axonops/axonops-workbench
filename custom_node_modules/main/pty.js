...
const Path = require('path'),
  /**
   * PTY creation module
   * Creates pseudo terminal, it returns a terminal object which allows interaction - reads and writes - with that terminal
   */
...
class Pty {
  constructor(window, data) {
    ...
    this.allOutput = '' // Store all output in order to catch CQLSH-STARTED keyword
    ...
    // Set a timeout function to be triggered if we didn't get any chunk of the request's result within 5 seconds
    ...
    let cleanCommand = (command) => {
      ...
      command = command.toLowerCase()
        // Get rid of an extra weird unwanted characters and symbols
    ...
    main.process.on('data', (data) => {
      try {
        ...
        try {
          // If one of the keyword have been captured already then skip this try-catch block
        ...
        /**
         * Handle the request - from the renderer thread - for getting a query tracing result
        ...
        /**
         * Handle the request - from the renderer thread - for sending the metadata info
  ...
  createCQLSHInstance(cqlshrcPath) {
    ...
    // If there're more arguments to be passed - like username and password -
    let moreArguments = ''
    try {
...
let testConnectionWithCluster = (window, data) => {
  ...
  let result = {
      connected: false, // Connection status - `false`: failed, `true`: successed - with the cluster
    ...
    sendTimeout = null,
    // All output from the testing process are appended here
    fullOutput = ''
  ...
  try {
    ...
    let checkOutput = (output) => {
      ...
      try {
        ...
        // If the version is `undefined` then it happened that the output split into multiple lines
...
let watchFileExistance = (filePath, callback, startTime = new Date().getTime()) => {
  try {
    // Once it's exists return `true` with the callback function
...
/**
 * Handle the creation of a Bash session inside Docker project
...
let bashSession = (window, data) => {
  ...
  process.write(`${Platform != 'win32' ? 'sudo ' : ''}docker-compose -p "${data.projectID}" exec "cassandra-0" /bin/bash` + NewLineChar)
