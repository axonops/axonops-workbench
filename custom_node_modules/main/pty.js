...
class Pty {
  ...
  createCQLSHInstance(data) {
    ...
    let pageCode = Platform == 'win32' ? 'chcp 65001 && ' : '',
       ...
       */
      command = `${pageCode}${binCall} ${override} --cqlshrc="${data.cqlshrcPath}" -SPLIT- ${moreArguments} --log="${this.logPath}" --varsManifest="${data.variables.manifest}" -SPLIT- --varsValues="${data.variables.values}" --workspaceID="${this.workspaceID}"`
...
let testConnectionWithCluster = (window, data) => {
  ...
  try {
    ...
    tempProcess.write(`${pageCode}${binCall} ${override} --cqlshrc="${data.cqlshrcPath}" ${moreArguments} --test 1 --varsManifest="${data.variables.manifest}" --varsValues="${data.variables.values}" --workspaceID="${data.workspaceID}"` + OS.EOL)
