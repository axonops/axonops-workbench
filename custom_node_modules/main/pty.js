...
class Pty {
  ...
  async createCQLSHInstance(data) {
    ...
    try {
      ...
      basicCommand = `${pageCode}${binCall} ${override} --cqlshrc="${data.cqlshrc || data.cqlshrcPath}" -SPLIT- ${moreArguments} --log="${this.logPath}" --varsManifest="${names.manifest}" -SPLIT- --varsValues="${names.values}" --workspaceID="${this.workspaceID}"`
    } catch (e) {}
