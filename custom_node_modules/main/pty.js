...
class Pty {
      ...
      try {
        ...
        try {
          ...
          if (isNaN(parseInt(e.toString()))) {
            log.error('Error in process terminal', e)
          }
        }
  ...
  command(command, blockID, currentBuffer, process = 'processMain') {
    ...
    if (OS.platform() != 'darwin') {
      try {
        ...
        log.error('Error sending command to the pty instance', {'process': process, 'command': command, 'error:': e})
      }
    }
  ...
  close() {
    ...
    try {
      ...
      log.warning('Something went wrong while closing PTY instance', e)
    }
  ...
  getMetadata(metadataPrintRequestID, currentBuffer) {
    ...
    watchFileExistance(tempFile, async (found) => {
      ...
      try {
        ...
        // If the error is a number then don't log the error
        if (isNaN(parseInt(e.toString()))) {
          log.error('Error deleting temporary file', {'path': tempFile, 'error': e})
        }
      }
    })
  }
  ...
  data.IPCMain.on(`pty:${data.id}:command:bash-session`, (_, command) => {
    try {
      ...
      try {
        ...
        log.error('Error sending command to PTY instance', e)
      } catch (e) {}
    }
  })
