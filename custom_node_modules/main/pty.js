...
class Pty {
    ...
    main.process.on('data', (data) => {
      try {
        ...
        try {
          ...
          if ((['KEYWORD:OUTPUT:COMPLETED:ALL', 'KEYWORD:CQLSH:STARTED', 'cqlsh>']).some((keyword) => main.allOutput.indexOf(minifyText(keyword)) != -1)) {
   ...
   * This is possible by passing the keyword `KEYWORD:STATEMENT:IGNORE-{digit}` which tells the cqlsh tool to ignore the given statement
   */
  clearBuffer() {
    try {
      this.command(`KEYWORD:STATEMENT:IGNORE-${Math.floor(Math.random() *  1001)}`)
    } catch (e) {}
  }
