...
class Pty {
  ...
  async createCQLSHInstance(data) {
    ...
    let pageCode = Platform == 'win32' ? 'chcp 65001 && ' : '',
       ...
       */
      command = `${pageCode}${binDirectory}${binCall} ${override} --cqlshrc="${data.cqlshrc || data.cqlshrcPath}" -SPLIT- ${moreArguments} --log="${this.logPath}" --varsManifest="${data.variables.manifest}" -SPLIT- --varsValues="${data.variables.values}" --workspaceID="${this.workspaceID}"`,
