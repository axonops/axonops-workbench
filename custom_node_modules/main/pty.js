...
class Pty {
  ...
  constructor(window, data) {
    ...
    let processesNames = ['processMain', 'processBasic']

    // Loop through the names and create the pty instances
    for (let processName of processesNames) {
      this[processName] = this.pty.spawn(Shell, [], {
        cwd: CWD,
  ...
  createCQLSHInstance(data) {
    ...
    let pageCode = Platform == 'win32' ? 'chcp 65001 && ' : '',
       ...
       */
      command = `${pageCode}${binCall} ${override} --cqlshrc="${data.cqlshrc || data.cqlshrcPath}" -SPLIT- ${moreArguments} --log="${this.logPath}" --varsManifest="${data.variables.manifest}" -SPLIT- --varsValues="${data.variables.values}" --workspaceID="${this.workspaceID}"`,
    ...
    try {
      ...
      this.command(command.replace(/\-SPLIT\-/g, '') + basicArgument, undefined, undefined, 'processBasic')
    } catch (e) {
       ...
       */
      commandSplit[commandSplit.length - 1] = commandSplit[commandSplit.length - 1].replace(keepTempArgument, '') + basicArgument

      // Loop through each portion and execute it
      commandSplit.forEach((portion, index) => this.command(`${portion}${(index + 1) >= commandSplit.length ? '' : ' \\'}`, undefined, undefined, 'processBasic'))
    }
  }
...
let testConnectionWithCluster = (window, data) => {
  ...
  try {
    ...
    tempProcess.write(`${pageCode}${binCall} ${override} --cqlshrc="${data.cqlshrc || data.cqlshrcPath}" ${moreArguments} --test 1 --varsManifest="${data.variables.manifest}" --varsValues="${data.variables.values}" --workspaceID="${data.workspaceID}"` + OS.EOL)
