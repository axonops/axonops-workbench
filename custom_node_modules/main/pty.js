...
class Pty {
  ...
  constructor(window, data) {
    ...
    /**
     * Set a timeout function to be triggered if we don't get any chunk of the request's result/response within 5 seconds
     * This process only executed with requests like get tracing query and cql description
     *
     * @Parameters:
     * {string} `requestID` the associated request's ID
     */
    let setRequestResultTimeOut = (requestID) => {
     ...
     * @Parameters:
     * {string} `?requestID` the associated request's ID
     * {boolean} `?reset` whether or not the timer shoud be reseted not fully terminated
     */
    let clearRequestResultTimeOut = (requestID = null, reset = false) => {
      // Clear the timer
      try {
      ...
      setRequestResultTimeOut(requestID)
    }
    ...
    main.process.on('data', (data) => {
      try {
        ...
        try {
          ...
          if ((['keyword:output-finished', 'keyword:cqlsh-started', 'cqlsh>']).some((keyword) => main.allOutput.indexOf(keyword) != -1)) {
            // Add the ignore keyword
            main.allOutput = 'ignore-text'

            // Send the `started` keyword to the renderer thread
            return main.window.webContents.send(`pty:data:${main.id}`, {
        ...
        try {
          ...
          clearRequestResultTimeOut(requestID, true)
          ...
          try {
            ...
            if ((new RegExp('OUTPUT\-FINISHED')).exec(data) == null)
              throw 0
        ...
        try {
          // If `metadataPrintRequestID` is empty then we may skip this try-catch block
          if (main.checkConnectivityRequestID.length <= 0)
            throw 0
           ...
           * Update `prepareToSendAgain` to `true`; to resume the sending process
           */
          main.checkConnectivityRequestID = ''
          main.prepareToSendAgain = true
        ...
        try {
          ...
          dataManipulated = manipulateOutput(cleanCommand(dataManipulated))
          latestCommand = manipulateOutput(cleanCommand(latestCommand))
        } catch (e) {
        ...
        main.window.webContents.send(`pty:data:${main.id}`, {
          output: data,
          blockID: main.latestBlockID
        })
      } catch (e) {}
    })
  }
   ...
   */
  command(command, blockID, currentBuffer) {
    ...
    // Whether or not the passed buffer is valid
    let isCurrentBufferValid = currentBuffer != undefined && currentBuffer.length > 0,
      // Get a random ID for the app's associated UI terminal's clearing request
      clearRequestID = RandomID(10)

    /**
     * Determine if there's a need to clear part of the current buffer - the current active line -
     *
     * This is important to prevent executing complete statements where there's already an incomplete buffer
     */
    try {
      // If there's no passed buffer then skip this try-catch block
      if (!isCurrentBufferValid)
        throw 0

      // Clear the current buffer of the pty instance
      this.clearBuffer()

      // Clear the associated app's UI terminal current line
      this.window.webContents.send(`terminal:clear-line`, {
        terminalID: this.terminalID,
        clearRequestID
      })

      // Once a response is received
      IPCMain.on(`terminal:clear-line:${clearRequestID}`, () => {
        // Send the command to the pty instance
        try {
          this.process.write(command + OS.EOL)
        } catch (e) {}

        // Write back the previous reserved buffer
        try {
          this.process.write(currentBuffer)
        } catch (e) {}
      })

      // Skip any upcoming code
      return
    } catch (e) {}
  ...
  /**
   * Clear the pty instance's current buffer; to make sure it's clean before executing a full command
   * This is possible by passing the keyword `KEYWORD:IGNORE-THIS-STATEMENT-{digit}` which tells the cqlsh tool to ignore the given statement
   */
  clearBuffer() {
    try {
      this.command(`KEYWORD:IGNORE-THIS-STATEMENT-${Math.floor(Math.random() *  1001)}`)
    } catch (e) {}
  }
   ...
   */
  resize(nums) {
    this.process.resize(nums.cols, nums.rows)
  }
  ...
  createCQLSHInstance(cqlshrcPath) {
    ...
    let pageCode = Platform == 'win32' ? 'chcp 65001 && ' : '',
       ...
       */
      command = `${pageCode}${binCall} ${override} --cqlshrc="${cqlshrcPath}" -SPLIT- ${moreArguments} --log="${this.logPath}" --varsManifest="${VariablesFilePath.manifest}" -SPLIT- --varsValues="${VariablesFilePath.values}" --workspaceID="${this.workspaceID}"`
    ...
    try {
      ...
      // Run the cqlsh's executing command and get rid of unwanted keywords
      this.command(command.replace(/\-SPLIT\-/g, ''))
    } catch (e) {
      ...
      commandSplit.forEach((portion, index) => this.command(`${portion}${(index + 1) >= commandSplit.length ? '' : ' \\'}`))
    }
  }
...
let testConnectionWithCluster = (window, data) => {
  ...
  try {
    ...
    tempProcess.write(`${pageCode}${binCall} ${override} --cqlshrc="${data.cqlshrcPath}" ${moreArguments} --test 1 --varsManifest="${VariablesFilePath.manifest}" --varsValues="${VariablesFilePath.values}" --workspaceID="${data.workspaceID}"` + OS.EOL)
    ...
    let checkOutput = (output) => {
      ...
      if (output.toLowerCase().indexOf('active code') <= 0 && output.length > 5 && ['bin', 'bash', 'zsh'].every((str) => output.indexOf(str) <= -1))
        lastOutput = output.replace('KEYWORD:TEST-COMPLETED', '')
      ...
      let sendResult = () => {
        ...
        window.webContents.send(`pty:test-connection:${data.requestID}`, {
          ...result,
          error: lastOutput.replace(/TEST\-COMPLETED/gm, '')
        })
...
let bashSession = (window, data) => {
  ...
  if (Platform == 'darwin')
    process.write(`export PATH=$PATH:/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/bin:/usr/bin:/usr/sbin:/sbin:/bin:/usr/sbin` + OS.EOL)
