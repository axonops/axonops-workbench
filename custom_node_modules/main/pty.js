...
class Pty {
  ...
  async createCQLSHInstance(data) {
     ...
     */
    let command = `${pageCode}${binDirectory}${binCall} ${override} --cqlshrc="${data.cqlshrc || data.cqlshrcPath}" -SPLIT- ${moreArguments} --log="${this.logPath}" --varsManifest="${data.variables.manifest}" -SPLIT- --varsValues="${data.variables.values}" --workspaceID="${this.workspaceID}"`,
    ...
    try {
      ...
      basicCommand = `${pageCode}${binDirectory}${binCall} ${override} --cqlshrc="${data.cqlshrc || data.cqlshrcPath}" -SPLIT- ${moreArguments} --log="${this.logPath}" --varsManifest="${names.manifest}" -SPLIT- --varsValues="${names.values}" --workspaceID="${this.workspaceID}"`
    } catch (e) {}
...
let testConnectionWithCluster = (window, data) => {
  ...
  try {
    ...
    tempProcess.write(`${pageCode}${binDirectory}${binCall} ${override} --cqlshrc="${data.cqlshrc || data.cqlshrcPath}" ${moreArguments} --test 1 --varsManifest="${data.variables.manifest}" --varsValues="${data.variables.values}" --workspaceID="${data.workspaceID}"` + OS.EOL)
