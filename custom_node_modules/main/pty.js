...
class Pty {
  ...
  async createCQLSHInstance(data) {
    // By default, cqlsh `v6.1.0` will be used to connect with the cluster
    let cqlsh = '410',
    ...
    cqlsh = version.startsWith('4.0') ? '407' : cqlsh
...
let runKeysGenerator = (callback) => {
  ...
  process.write('cd keys_generator && ./keys_generator' + OS.EOL)

  // When receiving any data from the instance send it to the provided window
  process.on('data', function(_data) {
    output += `${_data}`

    try {
      let publicKey = output.match(/-+\s*BEGIN\s*PUBLIC\s*KEY-+[\s\S]+-+END\s*PUBLIC\s*KEY-+/gi)[0]
    ...
    try {
      let processFinished = output.match(/.+(keys_generator.*)$/gi)[0]
...
let initializeCQLSH = (window) => {
  ...
  process.write('cd "cqlsh-407/" && ./cqlsh-407 --username=cassandra --password=cassandra --initialize=1 && cd .. && cd "cqlsh-410/" && ./cqlsh-410 --username=cassandra --password=cassandra --initialize=1' + OS.EOL)

  process.on('data', (data) => {
    output += `${data}`

    if ((manipulateOutput(output).match(/KEYWORD:INIT:COMPLETED/gi) || []).length >= 2)
