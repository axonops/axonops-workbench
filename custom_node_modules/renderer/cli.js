...

{
  IPCRenderer.on('cli:list-connections', async (_, workspaceID) => {
    let connections = await Modules.Clusters.getClusters(workspaceID)
...

{
  IPCRenderer.on('cli:import-connection', async (_, jsonObject) => {
    getKey('public', async (key) => {
      ...
      try {
        ...
        try {
          ...
          IPCRenderer.on(`pty:test-connection:${requestID}`, async (_, result) => {
            ...
            // Now everything should be fine and ready to be passed to the save function
            Modules.Clusters.saveCluster(workspaceID, {
              ...finalJSON
            }).then((result) => {
              // Send final result code
              sendResult({
       ...
       */
      let cqlshrcDefaultValues = await Modules.Clusters.getCQLSHRCContent(workspaceID, cqlshrcContent)
      ...
      // Connection ID
      try {
        contentJSON.basic.id = `cluster-${getRandomID(10)}`
      } catch (e) {}
      ...
      try {
        finalJSON.cqlshrc = Modules.Clusters.setCQLSHRCContent(cqlshrcDefaultValues, cqlshrcContent)
      } catch (e) {
        return sendResult({
          success: false,
          text: 'Failed to read and parse cqlsh.rc content'
