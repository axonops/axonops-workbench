...
let getMetadata = (clusterID, callback) => {
  try {
    addLog(`Get metadata as JSON string for the connection '${clusterID}' in workspace '${workspaceFolder}'`, 'process')
  } catch (e) {}
  ...
  IPCRenderer.on(`cluster:metadata:${metadataSendID}:${clusterID}`, (_, data) => {
    try {
      addLog(`Got the metadata of the connection '${clusterID}' in workspace '${workspaceFolder}', cleaning and repairing corruption`, 'process')
    } catch (e) {}

    try {
      addLog(`Metadata before the repairing process:`, 'info')
      addLog(data.metadata, 'info')
    } catch (e) {}
    ...
    try {
      ...
      try {
        addLog(`Metadata has been repaired:`, 'info')
        addLog(metadata, 'info')
      } catch (e) {}
      ...
      try {
        addLog(`Failed to repair the metadata, returned null:`, 'error')
        addLog(`${e}`, 'error')
        addLog(data.metadata, 'info')
      } catch (e) {}
