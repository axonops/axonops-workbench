 ...
 * As this module is only needed here it has been imported within the `clusters` module
 */
...
const Platform = OS.platform(),
  /**
   * Whether or not current platform is Windows
   * Linux and macOS are treated in the same way
   */
  IsWindows = Platform == 'win32',
  /**
   * Import this module only if the platform is Windows
   * This module is incomplete, after checking its source code it seems stable and it has what needed
   */
  RegEdit = Platform == 'win32' ? require('regedit-simple') : '',
  // Set the app's registries path - for Windows only -
  RegPath = 'HKLM\\SOFTWARE\\cassandraWorkbenchClustersSecrets'
...
let getClusters = async (workspaceID, rawData = false) => {
  ...
  try {
    ...
    // If the path to data file is not accessible then try to create a data file
    if (!pathIsAccessible(Path.join(workspaceFolder, 'clusters.json'))) {
      try {
        FS.writeFileSync(Path.join(workspaceFolder, 'clusters.json'), '')
      } catch (e) {}
    }
        ...
        try {
          ...
          try {
            ...
            if (!IsWindows) {
              ...
              secrets = await Keytar.findCredentials('cassandraWorkbenchClustersSecrets')
          ...
          try {
            ...
            // Get all attributes' names, and make sure they're within double quotes and start the attributes' values with double quote as well
            secret.password = secret.password.replace(/([aA-zZ]+)\:\b(?!"|')/gm, (_, matched) => {
              return `"${matched}":"`
            })

            // Complete the previous process by adding another double quote at the end of the attributes' values
...
let saveCluster = async (workspaceID, cluster) => {
  ...
  try {
    ...
    try {
      ...
      if (!IsWindows) {
        ...
        await Keytar.setPassword(
          'cassandraWorkbenchClustersSecrets',
    ...
    // Delete secrets from cluster object if exists; to prevent adding them in `clusters.json` and `info.json` files
  ...
  try {
    ...
    try {
      ...
      if (cluster.info.secrets != undefined && Object.keys(cluster.info.secrets).length > 0) {
        try {
          ...
          if (!IsWindows) {
            ...
            await Keytar.setPassword('cassandraWorkbenchClustersSecrets', cluster.info.id, JSON.stringify({
              ...cluster.info.secrets
            }))
      ...
      if (!IsWindows) {
        ...
        await Keytar.deletePassword('cassandraWorkbenchClustersSecrets', cluster.info.id)
  ...
  try {
    ...
    try {
      ...
      if (!IsWindows) {
        ...
        await Keytar.deletePassword('cassandraWorkbenchClustersSecrets', clusterID)
...
let getCQLSHRCContent = async (workspaceID, cqlshrc = null, editor = null, changeVariables = true) => {
  ...
  try {
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      // If the current option is actually a script hen rename `script` to something unique; because we can't add multiple values with the same name
      if (isScript)
      ...
      sections.forEach((section) => {
        ...
        for (let _option of options) {
          ...
          let option = result[section][_option],
            // Check if there're variables in that value
...
/**
 * Set `cqlsh.rc` file options' values to new ones based on a given sections array
 *
 * @Parameters:
 ...
 * {object} `?editor` the editor's object that can be used to get its content
 *
 * @Return: {string} the passed `cqlsh.rc` file content updated with the given sections array
 */
let setCQLSHRCContent = (sections, cqlshrc = null, editor = null) => {
  ...
  try {
    ...
    // Loop through the given sections
    Object.keys(sections).forEach((section) => {
      /**
       * Create a regular expression
       * It'll match the entire block of a given section
       */
 ...
 * @Parameters:
 * {string} `workspaceID` the ID of the target workspace
 * {string} `cqlshrc` a given `cqlsh.rc` file content
 ...
 */
let updateFilesVariables = async (workspaceID, cqlshrc, cqlshrcPath, removedVariables = null, changedVariables = null) => {
  let newContent = cqlshrc // Final content which be returned

  try {
    // Get the given `cqlsh.rc` content
    ...
    for (let section of sections) {
      ...
      try {
        ...
        for (let _option of options) {
          ...
          let option = content[section][_option]

          // Check if there're variables or more in the option's value
          try {
            let exists = removedVariables.filter(
              (variable) =>
              // Check the variable's name exists in the option's value
              option.search('${' + variable.name + '}') &&
              // Check that the variable's scope includes the given workspace
          ...
          content[section][_option] = option
        }
      } catch (e) {}
      ...
      try {
        ...
        for (let _option of options) {
          ...
          // Check where the old variable's name is being used - taking into account the old scope - and replace it with its old value
          for (let variable of changedVariables) {
            try {
              // Check the variable's old name existence in the option's value and the variable's old scope includes the given workspace
              let exists = option.search('${' + variable.name.old + '}') &&
                variable.scope.old.some((workspace) => [workspaceID, 'workspace-all'].includes(workspace))

              // If the variable doesn't exist in the current option's value then skip it and move to the next changed variable
              if (!exists)
                continue
              ...
              option = option.replace(regex, variable.value.old)
            } catch (e) {}
          }

          // Check where the new variables' values are in different options' values and replace them with the new variables' names
          for (let variable of changedVariables) {
            try {
              // Make sure the variable's scope includes the workspace, if not then skip that variable and move to the next one
              ...
              let regex = createRegex(`${variable.value.new}`, `gm`)
      ...
      // Last process is looping through options again and make sure all variables are set well
      try {
        // Get variables whose scope includes the workspace
        let variables = await getProperVariables(workspaceID)
        ...
        for (let _option of options) {
          ...
          try {
            ...
            let exists = variables.filter((variable) => option.search(variable.value))
    ...
    // Apply the new content on the given `cqlsh.rc` file content then write it to the `cqlsh.rc` file based on the given path `cqlshrcPath`
    newContent = await setCQLSHRCContent(content, cqlshrc)

    // Write the new content
    await FS.writeFileSync(cqlshrcPath, newContent)

    // Now attempt to update variables in the `ssh-tunnel.json` file
    try {
      // Get SSH tunneling info for the cluster using the given path for `cqlsh.rc` file
      let sshTunnelingInfo = await FS.readFileSync(
        Path.join(cqlshrcPath, '..', 'ssh-tunnel.json'), 'utf8')
      ...
      let sshValues = Object.keys(sshTunnelingInfo)

      for (let _sshValue of sshValues) {
        // Get the value for the current SSH option
        let value = sshTunnelingInfo[_sshValue]
        ...
        try {
          ...
          for (let variable of changedVariables) {
            try {
              ...
              value = value.replace(regex, variable.value.old)
            } catch (e) {}
          }
        ...
        // Last process is looping through SSH values again and make sure all variables are set well
        try {
          // Get variables whose scope includes the workspace
          let variables = await getProperVariables(workspaceID)

          try {
            let exists = variables.filter((variable) => value.search(variable.value))
    ...
    // If any error has occurred then return `false` or the `cqlsh.rc` content
    if (cqlshrcPath == null)
      return newContent
...
let getMetadata = (clusterID, callback) => {
   ...
   * The main thread will send the JSON string in parts, and they'll be grouped in the `clustersMetadata` array, then converted to JSON object using the `repairJSON` function
   */
...
let getQueryTracingResult = (clusterID, sessionID, callback) => {
  let result = '', // Final result which be returned
    requestTimeout = null, // Timeout to be triggered if we didn't get a valid result
    requestID = getRandomID(10) // Generate random ID for the request
  ...
  // Set timeout function to be triggered if we didn't get the final result within 5 seconds
  let setTimeOut = () => {
...
let getSnapshots = (clusterFolder, callback) => {
  ...
  FS.readdir(Path.join(clusterFolder, 'snapshots'), (err, snapshots) => {
    // If any error has occurred then return an ampty array
    if (err)
      try {
        return callback([])
      } catch (e) {}
    ...
    snapshots = snapshots.map((snapshot) => {
      ...
      // If the item is actually a file, and ends with `.json` then accept it, otherwise consider it as `null`
