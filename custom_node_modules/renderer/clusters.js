...
let deleteCluster = async (workspaceFolder, clusterFolder, clusterID, callback) => {
  ...
  try {
    ...
    await FS.writeFileSync(Path.join(clusterPath, '..', 'clusters.json'), applyJSONBeautify(clusters))
...
let getQueryTracingResult = (clusterID, sessionID, callback) => {
  ...
  // Set timeout function to be triggered if we don't get the final result within 5 seconds
  let setTimeOut = () => {
