...
const Platform = OS.platform(),
  ...
  RegPath = 'HKLM\\SOFTWARE\\AxonOpsDeveloperWorkbenchClustersSecrets'
        ...
        try {
          ...
          try {
            ...
            if (!IsWindows) {
              ...
              secrets = await Keytar.findCredentials('AxonOpsDeveloperWorkbenchClustersSecrets')
    ...
    try {
      ...
      if (!IsWindows) {
        ...
        await Keytar.setPassword(
          'AxonOpsDeveloperWorkbenchClustersSecrets',
  ...
  try {
    ...
    try {
      ...
      if (cluster.info.secrets != undefined && Object.keys(cluster.info.secrets).length > 0) {
        try {
          ...
          if (!IsWindows) {
            ...
            await Keytar.setPassword('AxonOpsDeveloperWorkbenchClustersSecrets', cluster.info.id, JSON.stringify({
              ...cluster.info.secrets
            }))
      ...
      if (!IsWindows) {
        ...
        await Keytar.deletePassword('AxonOpsDeveloperWorkbenchClustersSecrets', cluster.info.id)
  ...
  try {
    ...
    try {
      ...
      if (!IsWindows) {
        ...
        await Keytar.deletePassword('AxonOpsDeveloperWorkbenchClustersSecrets', clusterID)
...
let updateFilesVariables = async (workspaceID, cqlshrc, cqlshrcPath, removedVariables = null, changedVariables = null) => {
  ...
  try {
    // Get the passed `cqlsh.rc` content
      ...
      try {
        ...
        for (let _option of options) {
          ...
          for (let variable of changedVariables) {
            try {
              // Check the variable's old name existence in the option's value and the variable's old scope includes the passed workspace
 ...
 * {object} `terminal` the associated terminal's object
 ...
 */
let getMetadata = (clusterID, terminal, callback) => {
  /**
   * Get a random ID for that request
   * It'll be sent to the main thread
   */
  let metadataSendID = getRandomID(5),
    // Get the current statement/terminal's buffer
    currentBuffer = getTerminalCurrentBuffer(terminal)
 ...
 * @Return: {string} the target cluster's metadata in JSON string format
 */
let getCQLDescription = (clusterID, scope, terminal, callback) => {
  /**
   * Get a random ID for that request
   * It'll be sent to the main thread
   */
 ...
 */
let getQueryTracingResult = (clusterID, sessionID, terminal, callback) => {
 ...
 * @Return: {string} the target cluster's metadata in JSON string format
 */
let checkConnectivity = (clusterID, terminal, callback) => {
  /**
...
/**
 * Get a given terminal's current buffer - active line's buffer -
 *
 * @Parameters:
 * {object} `terminal` the aimed terminal's object
 * {boolean} `?noManipulation` whether or not the buffer should be manipulated
 * {boolean} `?lineAbove` get the current buffer of the line aobve the active one
 *
 * @Return: {string} the active line's buffer
 */
let getTerminalCurrentBuffer = (terminal, noManipulation = false, lineAbove = false) => {
  // Final result which be returned
  let statement = ''
  ...
  try {
     ...
     * https://xtermjs.org/docs/api/terminal/interfaces/ibuffernamespace/#readonly-active
     */
    let terminalBuffer = terminal.buffer.active

    // Get the entire written statement
    statement = terminalBuffer.getLine((terminalBuffer.baseY + terminalBuffer.cursorY) + (lineAbove ? -1 : 0)).translateToString(true)

    // If there's no need to manipulate the buffer then skip this try-catch block
    if (noManipulation)
      throw 0

    // Make sure to get rid of any keyword, and the prompt
...
/**
 * Clear a given terminal's current active line
 *
 * @Parameters:
 * {object} `terminal` the aimed terminal's object
 */
let clearTerminalCurrentLine = (terminal, requestID = '') => {
  /**
   * Get the terminal's active buffer
   * https://xtermjs.org/docs/api/terminal/interfaces/ibuffernamespace/#readonly-active
   */
  ...
  try {
    // Move the cursor to be at the beginning of the line
    terminal.write('\r')

    // Clear the current active line
    terminal.write('\x1b[2K')

    // Send a response to the main thread if needed
    if (requestID.length != 0)
      IPCRenderer.send(`terminal:clear-line:${requestID}`)
  } catch (e) {}
}
...
module.exports = {
  ...
  getSnapshots,
  getNewestSnapshot,
  // Operations regards the app's UI terminal
  getTerminalCurrentBuffer,
  clearTerminalCurrentLine
}
