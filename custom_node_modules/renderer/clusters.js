 ...
 */
let getTerminalCurrentBuffer = (terminal, noManipulation = false) => {
  // Final result which be returned
  let statement = ''
  ...
  try {
    ...
    statement = terminalBuffer.getLine(terminalBuffer.baseY + terminalBuffer.cursorY).translateToString(true)
    ...
    statement = statement.replace(new RegExp(`(^${OS.EOL}*)?KEYWORD\:[A-Z]+(\-[A-Z]+)?${OS.EOL}*`, 'gmi'), '')
      .replace(/^(.*?cqlsh.*?>)\s*/g, '')
  } catch (e) {}
