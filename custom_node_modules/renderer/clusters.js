...
let getClusters = async (workspaceID, rawData = false) => {
  ...
  log.info('Retrieving saved clusters...', {'workspace': workspaceID})
  ...
  try {
    ...
    try {
      ...
      log.warning('Failed to parse saved cluster information', {'workspace': workspaceID, 'error': e})
      savedClusters = []
    }
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            log.warning('Failed to find credentials', {'error': e})
          }
          ...
          log.warning('Failed to check if the cluster has secrets to be passed with', {'cluster': cluster.name, 'error': e})
        }
        ...
        try {
          ...
          try {
            ...
            log.warning('Failed to parse ssh tunnel information', {'workspace': workspaceID, 'error': e})
            sshTunnel = []
          }
          ...
          log.warning('Error checking ssh tunnel', {'workspace': workspaceID, 'error': e})
          sshTunnel = []
        }
    ...
    log.warning('Something went wrong while getting saved clusters', {'workspace': workspaceID, 'error': e})
  } finally {
...
let saveCluster = async (workspaceID, cluster) => {
  ...
  // Add log about this process
  log.info('Saving cluster...', {'workspace': workspaceID, 'cluster': cluster.name})
  ...
  try {
    ...
    if (cluster.info.secrets != undefined && Object.keys(cluster.info.secrets).length > 0) {
      try {
        ...
        log.warning('Failed to save cluster secrets in a keychain', {'workspace': workspaceID, 'cluster': cluster.name, 'error': e})
      }
    }
    ...
    log.warning('Failed to save cluster', {'workspace': workspaceID, 'cluster': cluster.name, 'error': e})
...
let updateCluster = async (workspaceID, cluster) => {
  ...
  // Add log about this process
  log.info('Updating сluster...', {'workspace': workspaceID, 'cluster': cluster.name})
  ...
  try {
    ...
    try {
      ...
      log.warning('Something went wrong while handling cluster secrets on update', {'workspace': workspaceID, 'cluster': cluster.name, 'error': e})
    }
    ...
    log.warning('Something went wrong while updating cluster', {'workspace': workspaceID, 'cluster': cluster.name, 'error': e})
...
let deleteCluster = async (workspaceFolder, clusterFolder, clusterID, callback, keepFiles = false) => {
  ...
  // Add log about this process
  log.info('Deleting cluster...', {'workspaceFolder': workspaceFolder, 'clusterID': clusterID})
  ...
  try {
    ...
    try {
      ...
      log.warning('Failed to parse saved cluster data', {'path': clusterPath, 'file': 'connections.json', 'error': e})
      clusters = []
    }
    ...
    try {
      ...
      log.warning('Failed to delete secrets associated with the deleted cluster', {'clusterID': clusterID, 'error': e})
    }

    // Call the `callback` function with `true` as the deletion process was successfully completed
    callback(true)
  } catch (e) {
    log.warning('Failed to delete cluster', {'error': e, 'clusterID': clusterID})
...
let getSSHTunnelingInfo = async (workspaceID, clusterFolder) => {
  ...
  try {
    ...
    log.warning('Failed to get SSH tunneling info', {'workspace': workspaceID, 'clusterFolder': clusterFolder, 'error': e})
  } finally {
    // Return the final result
    return result
  }
  ...
  FS.readdir(Path.join(clusterFolder, 'snapshots'), (err, snapshots) => {
    ...
    snapshots = snapshots.map((snapshot) => {
      try {
        ...
        log.warning('Something went wrong while processing a snapshot', {'clusterFolder': clusterFolder, 'snapshot': snapshot, 'error': e})
      }
    }).filter((snapshot) => {
  ...
  getSnapshots(clusterFolder, async (snapshots) => {
    try {
      ...
      try {
        ...
        log.warning('Failed to get snapshot content', {'clusterFolder': clusterFolder, 'error': e})
      }
    } catch (e) {} finally {
      // Return the final result
      try {
        ...
        log.warning('Something went wrong executing callback for the latest snapshot', {'clusterFolder': clusterFolder, 'error': e})
      }
    }
  })
}
