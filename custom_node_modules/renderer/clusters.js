...
let getClusters = async (workspaceID, rawData = false) => {
  ...
  try {
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            log.error('Failed to find credentials', e)
          }
          ...
          log.error('Failed to check if the cluster has secrets to be passed with', {'cluster': cluster.name, 'error': e})
        }
        ...
        try {
          ...
          log.error('Error checking ssh tunnel', {'workspace': workspaceID, 'error': e})
          sshTunnel = []
        }
    ...
    log.error('Something went wrong while getting saved clusters', {'workspace': workspaceID, 'error': e})
  } finally {
...
let saveCluster = async (workspaceID, cluster) => {
  ...
  try {
    ...
    log.error('Failed to save cluster', {'workspace': workspaceID, 'cluster': cluster.name, 'error': e})
...
let deleteCluster = async (workspaceFolder, clusterFolder, clusterID, callback, keepFiles = false) => {
  ...
  try {
    ...
    log.error('Failed to delete cluster', {'error': e, 'clusterID': clusterID})
  ...
  getSnapshots(clusterFolder, async (snapshots) => {
    try {
      ...
      try {
        ...
        log.error('Failed to get snapshot content', {'clusterFolder': clusterFolder, 'error': e})
      }
    } catch (e) {} finally {
      // Return the final result
      try {
        ...
        log.error('Something went wrong executing callback for the latest snapshot', {'clusterFolder': clusterFolder, 'error': e})
      }
    }
  })
}
