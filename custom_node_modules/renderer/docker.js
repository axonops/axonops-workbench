...
class DockerCompose {
  ...
  async createDockerComposeYAML(cassandraVersion = '4.0', numOfNodes = 3) {
    ...
    const DockerContainersPath = Path.join(await getUserDataDir(), 'docker')
    const DockerContainersConfigPath = Path.join(await getUserDataDir(), 'docker', 'docker.json')
    // Add log about this process
    try {
      addLog(`Create docker compose config file with Cassandra ™ v${cassandraVersion} and ${numOfNodes} node(s)`, 'process')
    } catch (e) {}
    ...
    try {
      await FS.mkdirSync(Path.join(DockerContainersPath, this.folderName), { recursive: true })
    } catch (e) {
      try {
        errorLog(e, 'docker')
      } catch (e) {}
  ...
  async startDockerCompose(pinnedToastID, callback, containerName = null) {
    ...
    console.log(DockerContainersPath)
    ...
    Terminal.spawn(`cd "${Path.join(DockerContainersPath, this.folderName)}" && docker-compose -p "cassandra_${containerName}" up -d`, pinnedToastID, 'start', (error, stdout, stderr) => {
      ...
      // Add log about this process
      try {
        addLog(`Output of running docker/sandbox project '${Path.join(DockerContainersPath, this.folderName)}' under name 'cassandra_${containerName}' is:
   ...
   * The name of the container can be passed, if it hasn't then the instance's container's name will be used
   */
  async stopDockerCompose(pinnedToastID, callback, containerName = null) {
    // Set the docker container's default path
    const DockerContainersPath = Path.join(await getUserDataDir(), 'docker')
    const DockerContainersConfigPath = Path.join(await getUserDataDir(), 'docker', 'docker.json')
    // Set the container's name, or use the instance's one
    ...
    Terminal.spawn(`cd "${Path.join(DockerContainersPath, this.folderName)}" && docker-compose -p "cassandra_${this.containerName}" down`, pinnedToastID, 'stop', (error, stdout, stderr) => {
      ...
      // Add log about this process
      try {
        addLog(`Output of stopping docker/sandbox project '${Path.join(DockerContainersPath, this.folderName)}' under name 'cassandra_${containerName}' is:
          ${stdout}.${finalError != undefined ? ' Possible error: ' + finalError : ''}`)
      } catch (e) {}
...
let getProjects = async () => {
  ...
  // Add log about this process
  try {
    addLog(`Retrieve saved docker/sandbox projects`, 'process')
  } catch (e) {}

  try {
    // Get all workspaces
    let savedProjects = await FS.readFileSync(DockerContainersConfigPath, 'utf8')
...
let saveProject = async (project) => {
  ...
  try {
    if (!FS.existsSync(DockerContainersPath)) {
      await FS.mkdirSync(DockerContainersPath, { recursive: true });
    }
  } catch (e) {
    console.log(e)
  }
  // Add log about this process
  try {
    addLog(`Save a docker/sandbox project with info '${JSON.stringify(project)}'`, 'process')
  } catch (e) {
    console.log(e)
  }
  ...
  try {
    ...
    console.log(`Reading docker config from ${DockerContainersConfigPath}`)

    let projects = []
    try {
      projects = await FS.readFileSync(DockerContainersConfigPath, 'utf8')
    } catch(e) {
      console.log(e)
    }
    ...
    try {
      ...
      console.log(`Could not parse ${DockerContainersConfigPath} file`)
      projects = []
    }
    ...
    try {
      await FS.writeFileSync(DockerContainersConfigPath, applyJSONBeautify(projects))
    } catch(e) {
      console.log(`Error writing to ${DockerContainersConfigPath}: ${e}`)
      return 0
    }
...
let deleteProject = async (folderName) => {
  ...
  // Add log about this process
  try {
    addLog(`Delete docker/sandbox project in path '${Path.join(DockerContainersPath, folderName)}'`, 'process')
  } catch (e) {}
  ...
  try {
    ...
    let projects = await FS.readFileSync(DockerContainersConfigPath, 'utf8')
    ...
    try {
      await FS.writeFileSync(DockerContainersConfigPath, applyJSONBeautify(projects), 'utf8')
    } catch (e) {
      try {
        errorLog(e, 'docker')
      } catch (e) {}
 ...
 */
{
  ...
  Terminal.run = (...args) => {
    ...
    if (OS.platform == 'darwin')
      args[0] = `export PATH=$PATH:/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin && ${args[0]}`
