...
class DockerCompose {
  ...
  async createDockerComposeYAML(cassandraVersion = '4.0', installAxonOps = true, numOfNodes = 3) {
    ...
    log.info('Creating local cluster config file...', 'cassandra version', cassandraVersion, 'nodes', numOfNodes)
  ...
  async startDockerCompose(pinnedToastID, callback, containerName = null) {
    ...
    Terminal.spawn(`cd "${Path.join(DockerContainersPath, this.folderName)}" && ${dockerComposeBinary} ${extraArgument} -p "cassandra_${containerName}" up -d`, pinnedToastID, 'start', (error, stdout, stderr) => {
      ...
      // Add log about this process
      log.info('Running local cluster', 'name', containerName, 'path', Path.join(DockerContainersPath, this.folderName))
      log.info('Output', `${stdout}.${finalError != undefined ? ' Possible error: ' + finalError : ''}`)
  ...
  stopDockerCompose(pinnedToastID, callback, containerName = null) {
    ...
    // Add log about this process
    log.info('Stopping local cluster...', `cassandra_${containerName}`, Path.join(DockerContainersPath, this.folderName))
    ...
    Terminal.spawn(`cd "${Path.join(DockerContainersPath, this.folderName)}" && ${dockerComposeBinary} ${extraArgument} -p "cassandra_${this.containerName}" down`, pinnedToastID, 'stop', (error, stdout, stderr) => {
      ...
      // Add log about this process
      log.info('Stopping local cluster', 'name', containerName, 'path', Path.join(DockerContainersPath, this.folderName))
      log.info('Output', `${stdout}.${finalError != undefined ? ' Possible error: ' + finalError : ''}`)
...
let checkDockerCompose = (callback) => {
  ...
  let checkProcess = (command, callback) => Terminal.run(command, (err, data, stderr) => {
      // Add log about this process
      try {
        log.info('Checking if docker and docker-compose available...', command)
        log.debug('Err output', err)
        log.debug('Data output', err)
        log.debug('Stderr output', err)
      } catch (e) {}
...
let checkCassandraInContainer = (pinnedToastID, port, callback, timestamp = null, requestID = null, send = true) => {
  ...
  // Add log about this process
  log.info('Check Cassandra is ready to be connected with', port)
  ...
  IPCRenderer.on(`pty:test-connection:${requestID}`, (_, result) => {
    ...
    if (result.connected && result.version != undefined) {
      ...
      log.info('Cassandra is ready', port)
    ...
    log.info('message', message, 'port', port)
...
let getPortsFromYAMLFile = async (folderName) => {
  ...
  // Add log about this process
  log.info('Retrieve ports of AxonOps agent and Apache Cassandra', Path.join(DockerContainersPath, folderName, 'docker-compose.yml'), JSON.stringify(ports))
...
let getProjects = async () => {
  ...
  try {
    ...
    try {
      ...
      log.error('Failed to parse saved projects', Path.join(DockerContainersPath, 'localclusters.json'), e)
      savedProjects = []
    }
...
let saveProject = async (project) => {
  ...
  // Add log about this process
  log.info('Saving local cluster...', JSON.stringify(project))
...
let deleteProject = async (folderName, keepFiles = false) => {
  ...
  // Add log about this process
  log.info('Deleting local cluster...', Path.join(DockerContainersPath, folderName))
