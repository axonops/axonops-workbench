    ...
    // Provide unique port for each Cassandra node in the project
    {
      try {
        ...
        let latestYamlObject = YAML.load(yamlContent),
          ...
          nodes = Object.keys(latestYamlObject.services).filter((service) => service.startsWith('cassandra-') && service != 'cassandra-0')

        // If the project has only one node then skip this try-catch block
        if (nodes.length <= 0)
          throw 0
        ...
        for (let node of nodes) {
          // Get a unique port
          let nodePort = await getRandomPort()

          // Set the port in format `{Unique Port}:{9043, 9044, 9045, 9043 + n}`
          latestYamlObject.services[node].ports[0] = `${nodePort}:${9043 + index}`;
