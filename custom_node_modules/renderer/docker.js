...
class DockerCompose {
  ...
  async createDockerComposeYAML(cassandraVersion = '4.0', installAxonOps = true, numOfNodes = 3) {
    ...
    try {
      ...
      log.error('Failed to create snapshots folder', {'folder': this.folderName, 'error': e})
    }
...
let getPortsFromYAMLFile = async (folderName) => {
  ...
  try {
    ...
    try {
      ...
      log.warning('Failed to extract axon-dash port', e)
    }
    ...
    try {
      ...
      log.warning('Failed to extract cassandra port', e)
    }
  } catch (e) {
    log.warning('Failed to get ports from docker-compose file', e)
  }
