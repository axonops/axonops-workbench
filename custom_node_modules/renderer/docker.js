...
let checkDockerCompose = (callback) => {
  ...
  Terminal.run('docker-compose -v', (err, stderr, data) => {
    ...
    let isDockerComposeInstalled = !err || `${err}`.indexOf('docker-compose version') != -1

    // If the host OS is Windows then there's no need to check for `docker` group and just call the callback function
    ...
    Terminal.run('groups', (err, stderr, data) => {
      ...
      // Call the callback function with final result
