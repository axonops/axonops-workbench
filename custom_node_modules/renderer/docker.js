...
const DockerContainersPath = Path.join(__dirname, '..', '..', 'data', 'docker')
...
class DockerCompose {
  ...
  constructor(folderName = null) {
     ...
     */
    this.folderName = folderName || getRandomID(25)
   ...
   */
  async createDockerComposeYAML(cassandraVersion = '4.0', numOfNodes = 3) {
    ...
    try {
      // If the numebr of nodes is `3` then skip this try-catch block; as this is the default case already
      if (numOfNodes == 3)
        throw 0
      ...
      let yamlObject = YAML.load(yamlContent),
        ...
        cassandraNodeInfo = {
          services: {
            ...
            axonAgent: {
              ...yamlObject.services['axon-agent-1']
            }
          },
          volumes: {
      ...
      Object.keys(yamlObject.services).filter((service) => service.startsWith('cassandra-')).forEach((service) => {
        ...
        delete yamlObject.services[`axon-agent-${number}`]
      ...
      /**
       * Loop based on the give number of nodes
       * The loop starts from `1`; as the first node `0` is always exists no matter what the number of nodes is
       */
      for (let i = 1; i < numOfNodes; i++) {
        ...
        // Set AxonOps Agent service
        yamlObject.services[`axon-agent-${i}`] = this.applyCassandraNodeID(cassandraNodeInfo.services.axonAgent, i)
      ...
      // Convert YAML JSON object to string
    ...
    // Create the container's folder if not already existed
    try {
      await FS.mkdirSync(Path.join(DockerContainersPath, this.folderName))
    } catch (e) {}
  ...
  applyCassandraNodeID(content, ID) {
    // Convert the given JSON content from object to string
    content = JSON.stringify(content)

    // Set the given ID in the suitable positions
    content = content.replace(/\-1/gm, `-${ID}`)

    // Convert the JSON object after the manipulation process to string
  ...
  async startDockerCompose(pinnedToastID, callback, containerName = null) {
    ...
    // Execute the `docker-compose` command with root/administrator privileges
    SudoPrompt.spawn(`cd "${Path.join(DockerContainersPath, this.folderName)}" && docker-compose -p "cassandra_${containerName}" up -d`, pinnedToastID, {
      name: 'Docker Projct'
  ...
  stopDockerCompose(pinnedToastID, callback, containerName = null) {
    ...
    // Execute the `docker-compose` command with root/administrator privileges
    SudoPrompt.spawn(`cd "${Path.join(DockerContainersPath, this.folderName)}" && docker-compose -p "cassandra_${this.containerName}" down`, pinnedToastID, {
      name: 'Docker Project'
...
/**
 * Check if the host machine has a docker compose installed
 ...
 */
let checkDockerCompose = (callback) => Terminal.run('docker-compose -v', (err, stderr, data) => callback(!err || `${err}`.indexOf('docker-compose version') != -1))
 ...
 *
 * @Return: {object} AxonOps agent and Apache Cassandra ports in the given YAML file
 */
let getPortsFromYAMLFile = async (folderName) => {
  ...
  try {
    // Read and load the given YAML file
 ...
 *
 * @Parameters:
 * {integer} `port` the port in the host machine which connects to Cassandra in the container
 * {object} `callback` function that will be triggered with passing the final result
 *
 * Other parameters are for an internal use only
...
let checkCassandraInContainer = (pinnedToastID, port, callback, timestamp = null, requestID = null, send = true) => {
  ...
  // Same treat to the timestamp, it won't be null at all
  timestamp = timestamp || new Date().getTime()

  // If about 2 minutes have passed and there's no result about Cassandra then return a `false` result
  if (new Date().getTime() - timestamp >= 120000) {
  ...
  IPCRenderer.on(`pty:test-connection:${requestID}`, (_, result) => {
    ...
    // If received result is telling that Cassandra is ready then call the callback function and finish the process
    ...
    // Call the update function again but with `destroy` passed with `true`
    updatePinnedToast(pinnedToastID, '++Cassandra is not ready yet, recheck again in 20 seconds.')

    /**
     * Call the function again in a recursive loop
     * Calling it after 20 seconds is to give the main thread enough time to terminate the PTY instance and get rid of artifacts
     */
    setTimeout(() => checkCassandraInContainer(pinnedToastID, port, callback, timestamp, requestID, send), 20000)
  })
}
...
let getProjects = async () => {
  ...
  try {
    // Get all workspaces
    let savedProjects = await FS.readFileSync(Path.join(DockerContainersPath, 'docker.json'), 'utf8')
...
let saveProject = async (project) => {
  ...
  try {
    ...
    let projects = await FS.readFileSync(Path.join(DockerContainersPath, 'docker.json'), 'utf8')
    ...
    await FS.writeFileSync(Path.join(DockerContainersPath, 'docker.json'), applyJSONBeautify(projects))
 ...
 */
let deleteProject = async (folderName) => {
  ...
  try {
    ...
    // Filter the saved projects and exclude the one which wanted to be deleted
    projects = projects.filter((project) => project.folder != folderName)

    // Update the saved project's JSON file
    try {
      await FS.writeFileSync(Path.join(DockerContainersPath, 'docker.json'), applyJSONBeautify(projects), 'utf8')
    } catch (e) {}
...
let getDockerInstance = (clusterElement) => {
  ...
  let folderName = getAttributes(clusterElement, 'data-folder') || getAttributes(clusterElement, 'data-id')
 ...
 * {object} `options` addational options to be passed to the `Sudo Prompt` module
 ...
 */
SudoPrompt.spawn = (command, pinnedToastID, options, process, callback) => {
  /**
  ...
  let tempFileName = Path.join(OS.tmpdir(), `${getRandomID(10)}.tmp`),
    ...
    outputWatcher,
    // Define the inital command's output
    commandOutput = ''
  ...
  let checkFileCreated = () => {
    setTimeout(() => {
      // Get this watching process' starting time
      ...
      FS.exists(tempFileName, (exists) => {
        // If the file is not exist and the process hasn't exceeded 1 minute then keep the process alive
  ...
  // Call the file's existence wathcer
  checkFileCreated()

  // Update the passed command by dumping its output to the temporary file
  command = `${command} > ${tempFileName} 2>&1`

  // By default there's no need to change the file's permissions
  let changeFilePermissions = ''

  // If the platform is not Windows then add new commands
  if (OS.platform() != 'win32')
    changeFilePermissions = ` ; chmod 777 "${tempFileName}" && chown ${OS.userInfo().username} "${tempFileName}"`

  // Execute the manipulated command
  SudoPrompt.exec(command + changeFilePermissions, options, (error, stdout, stderr) => {
    // Once the execution finish
    FS.readFile(tempFileName, 'utf8', (_, content) => {
      try {
        // The process is `start` and the app needs to tell the user about that
        ...
        // The process is `stop` and no need to keep the pinned toast
