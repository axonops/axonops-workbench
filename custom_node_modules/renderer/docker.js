 ...
 * Will be updated with `docker-compose`, or Podman tool if needed
 */
...
class DockerCompose {
  ...
  async createDockerComposeYAML(cassandraVersion = '4.0', installAxonOps = true, numOfNodes = 3) {
    ...
    log.info('Creating local cluster config file...', {'version': cassandraVersion, 'nodes': numOfNodes})

    // Get a random free-to-use ports for the AxonOps agent and Apache Cassandra
    this.axonopsPort = await getRandomPort()
    this.cassandraPort = await getRandomPort()
    ...
    try {
      ...
      log.warning('Failed to process non-default nodes number', {'error': e})
    }
    ...
    // Provide unique port for each Cassandra node in the project
    {
      try {
        ...
        log.warning('Failed to provide ports', {'error': e})
      }
    }
    ...
    try {
      ...
      log.warning('Failed to create snapshots folder', {'folder': this.folderName, 'error': e})
    }
    ...
    try {
      ...
      log.warning('Failed to write docker-compose file', {'folder': this.folderName, 'error': e})
    }
  ...
  async startDockerCompose(pinnedToastID, callback, containerName = null) {
    ...
    // Add log about this process
    log.info('Starting local cluster...', {'path': Path.join(DockerContainersPath, this.folderName), 'name': `cassandra_${containerName}`})

    let extraArgument = `${dockerComposeBinary}` == 'podman-compose' ? `--podman-args="--log-level info"` : ''
    ...
    Terminal.spawn(`cd "${Path.join(DockerContainersPath, this.folderName)}" && ${dockerComposeBinary} ${extraArgument} -p "cassandra_${containerName}" up -d`, pinnedToastID, 'start', (error, stdout, stderr) => {
      ...
      // Add log about this process
      log.info('Attempted to run a local cluster', {
        'name': containerName,
        'status': true == status ? 'success' : 'failure',
        'path': Path.join(DockerContainersPath, this.folderName),
        'Output': `${stdout}.${finalError != undefined ? ' Possible error: ' + finalError : ''}`
      })
  ...
  stopDockerCompose(pinnedToastID, callback, containerName = null) {
    ...
    // Add log about this process
    log.info('Stopping local cluster...', {'name': `cassandra_${containerName}`, 'path': Path.join(DockerContainersPath, this.folderName)})
    ...
    Terminal.spawn(`cd "${Path.join(DockerContainersPath, this.folderName)}" && ${dockerComposeBinary} ${extraArgument} -p "cassandra_${this.containerName}" down`, pinnedToastID, 'stop', (error, stdout, stderr) => {
      ...
      // Add log about this process
      log.info('Local cluster stopped', {
        'name': containerName,
        'path': Path.join(DockerContainersPath, this.folderName),
        'Output': `${stdout}.${finalError != undefined ? ' Possible error: ' + finalError : ''}`
      })
...
let checkDockerCompose = (callback) => {
  ...
  let checkProcess = (command, callback) => Terminal.run(command, (err, data, stderr) => {
      // Add log about this process
        log.info('Checking if docker and docker-compose available...', {
          'command': command,
          'err': err,
          'data': data,
          'stderr': stderr,
    ...
    postCheck = (isDockerComposeInstalled, selectedManagementTool) => {
      ...
      Terminal.run('groups', (err, data, stderr) => {
        ...
        // Add log about this process
        log.info('Check the current user is in the docker group', output)
  ...
  Modules.Config.getConfig((config) => {
    ...
    try {
      ...
      checkProcess('podman compose version', (exist) => {
        ...
        /**
         * Reaching here means `podman compose` doesn't exist
         *
         * Set `podman-compose`
         *
         * Update the associated variable
        ...
        checkProcess('podman -v', (exist) => postCheck(exist, selectedManagementTool))
      })
    ...
    checkProcess('docker compose version', (exist) => {
      ...
      // Check the binary and call the post checking process function
      checkProcess('docker-compose -v', (exist) => postCheck(exist, selectedManagementTool))
    })
  })
}
...
let checkCassandraInContainer = (pinnedToastID, port, callback, timestamp = null, requestID = null, send = true) => {
  ...
  // Add log about this process
  log.info('Check if Cassandra is ready to be connected with', {'port': port})
  ...
  IPCRenderer.on(`pty:test-connection:${requestID}`, (_, result) => {
    ...
    if (result.connected && result.version != undefined) {
      ...
      log.info('Cassandra is ready', {'port': port})
    ...
    log.info('message', {'text': message, 'port': port})
...
let getPortsFromYAMLFile = async (folderName) => {
  ...
  try {
    ...
    try {
      ...
      log.warning('Failed to extract axon-dash port', {'error': e})
    }
    ...
    try {
      ...
      log.warning('Failed to extract cassandra port', {'error': e})
    }
  } catch (e) {
    log.warning('Failed to get ports from docker-compose file', {'error': e})
  }

  // Add log about this process
  log.info('Retrieved ports of AxonOps agent and Apache Cassandra', {'path': Path.join(DockerContainersPath, folderName, 'docker-compose.yml'), 'ports': JSON.stringify(ports)})
...
let getProjects = async () => {
  ...
  // Add log about this process
  log.info('Retrieving saved local clusters...')
  ...
  try {
    ...
    try {
      ...
      log.warning('Failed to parse saved projects', {'path': Path.join(DockerContainersPath, 'localclusters.json'), 'error': e})
      savedProjects = []
    }
    ...
    log.warning('Failed to retrieve saved docker projects', {'error': e})
  } finally {
    // Return workspaces
    return projects
  }
...
let saveProject = async (project) => {
  ...
  // Add log about this process
  log.info('Saving local cluster...', {'project': JSON.stringify(project)})
  ...
  try {
    ...
    log.warning('Something went wrong saving local project', {'error': e})
  } finally {
...
let deleteProject = async (folderName, keepFiles = false) => {
  ...
  // Add log about this process
  log.info('Deleting local cluster...', {'path': Path.join(DockerContainersPath, folderName)})
  ...
  try {
    ...
    try {
      ...
      log.warning('Failed to parse json file', {'path': Path.join(DockerContainersPath, 'localclusters.json'), 'error': e})
      projects = []
    }
    ...
    try {
      ...
      log.warning('Failed to write file', {'path': Path.join(DockerContainersPath, 'localclusters.json'), 'error': e})
    }

    // The project has been successfully deleted
    status = true
  } catch (e) {
    log.warning('Failed to delete local cluster', {'path': Path.join(DockerContainersPath, folderName), 'error': e})
  } finally {
    // Return final status
    return status
  }
 ...
 */
{
  // Copy the original `node-cmd` module
  let originalObject = {
    ...Terminal
  }
