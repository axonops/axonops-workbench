  ...
  try {
    ...
    let AppInfo = require(Path.join(__dirname, '..', '..', 'package.json')),
      buildInfo = {
        provider: "github",
        owner: "axonops",
        repo: "axonops-workbench-cassandra"
      },
      targetInfo = {
        ...
        try {
          ...
          try {
            if (`${release.name}`.startsWith('v'))
              release.name = `${release.name}`.slice(1)
          } catch (e) {}

          result.updateAvailable = (`${release.name}` != `${targetInfo.currentVersion}`)
          result.dismissed = (`${release.name}` == `${Store.get('dismissUpdate')}`)
