...
let saveWorkspace = async (workspace) => {
  ...
  try {
    ...
    let workspaces = []
    try {
      workspaces = await FS.readFileSync(Path.join(UserDataDir, 'workspaces.json'), 'utf8')
    } catch(e) {
      addLog(`Cannot read ${Path.join(UserDataDir, 'workspaces.json')}`, 'error')
    }
    ...
    let workspacePath = Path.join(UserDataDir, 'workspaces', folder)
    ...
    await FS.writeFileSync(Path.join(UserDataDir, 'workspaces.json'), applyJSONBeautify(workspaces))
