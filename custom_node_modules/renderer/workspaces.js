...
let getWorkspaces = async () => {
  ...
  try {
    // Get all workspaces
    let savedWorkspaces = await FS.readFileSync(Path.join((extraResourcesPath != null ? Path.join(extraResourcesPath) : Path.join(__dirname, '..', '..')), 'data', 'workspaces', 'workspaces.json'), 'utf8')

    // Convert the JSON content from string to object
    try {
      savedWorkspaces = JSON.parse(savedWorkspaces)
    } catch (e) {
      log.warning('Failed to parse saved workspaces', e)
      savedWorkspaces = []
    }
...
let saveWorkspace = async (workspace) => {
  ...
  // Add log about this process
  log.info('Saving workspace...', JSON.stringify(workspace))
...
let updateWorkspace = async (workspace) => {
  ...
  // Add log about this process
  log.info('Updating workspace...', JSON.stringify(workspace))
...
let deleteWorkspace = async (workspace, workspaces, keepFiles = false) => {
  ...
  // Add log about this process
  log.info('Delete workspace with attributes...', JSON.stringify(workspace))
