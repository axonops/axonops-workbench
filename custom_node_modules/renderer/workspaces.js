...
let getWorkspaces = async () => {
  ...
  log.info('Retrieving saved workspaces...')

  try {
    // Get all workspaces
    let workspacesPath = Path.join((extraResourcesPath != null ? Path.join(extraResourcesPath) : Path.join(__dirname, '..', '..')), 'data', 'workspaces', 'workspaces.json')
    let savedWorkspaces = await FS.readFileSync(workspacesPath, 'utf8')

    // Convert the JSON content from string to object
    try {
      savedWorkspaces = JSON.parse(savedWorkspaces)
    } catch (e) {
      log.warning('Failed to parse saved workspaces', {'path': workspacesPath, 'error': e})
      savedWorkspaces = []
    }
    ...
    // Loop through each workspace and get `clusters.json` file inside each one
    for (let workspace of workspaces) {
      try {
        ...
        let folderPath = !workspace.defaultPath ? workspace.path : Path.join((extraResourcesPath != null ? Path.join(extraResourcesPath) : Path.join(__dirname, '..', '..')), 'data', 'workspaces'),
          ...
          clusters = await FS.readFileSync(Path.join(folderPath, workspace.folder, 'connections.json'), 'utf8')

        // Convert the JSON content from string to object
        try {
          clusters = JSON.parse(clusters)
        } catch (e) {
          log.warning('Failed to parse saved clusters', {'workspace': workspace.name, 'error': e})
          clusters = []
        }
        ...
        log.warning('Failed to get clusters for workspace', {'workspace': workspace.name,'error': e})
      }
    }
  } catch (e) {
    log.warning('Failed to load saved workspaces', {'path': workspacesPath, 'error': e})
    // TODO: We really should show a warning toast to a user here 
  } finally {
    // Return workspaces
    return workspaces
  }
...
let saveWorkspace = async (workspace) => {
  ...
  // Add log about this process
  log.info('Saving workspace...', {'workspace': workspace.name})
  ...
  try {
    ...
    try {
      ...
      log.warning('Failed to parse saved workspaces', {'error': e})
      workspaces = []
    }
    ...
    try {
      delete workspace.clustersPath
      delete workspace.checkedClusters
    } catch (e) {}
    ...
    // Handle if the process for an imported cluster not a new one
    try {
      // If this attribute hasn't been found then skip this try-catch block
      if (originalWorkspace.clustersPath == undefined)
        throw 0

      // Copy the workspace's folder to the new location - the default path -
      await FS.copySync(originalWorkspace.clustersPath, workspacePath, {
        overwrite: true
      })

      // Get all saved clusters in the about-to-imported workspace
      let clustersManifest = await FS.readFileSync(Path.join(workspacePath, 'connections.json'), 'utf8')

      // Convert the manifest to Object
      try {
        clustersManifest = JSON.parse(clustersManifest)
      } catch (e) {
        log.warning('Failed to parse saved connections', {'error': e})
        clustersManifest = []
      }

      // Loop through each saved cluster
      for (let cluster of clustersManifest) {
        // If this cluster is meant to be imported then skip it
        if (originalWorkspace.checkedClusters.some((checkedCluster) => checkedCluster == cluster.folder))
          continue

        // Remove the cluster from the manifest
        clustersManifest = clustersManifest.filter((savedCluster) => savedCluster.folder != cluster.folder)

        // Remove its folder as well
        await FS.removeSync(Path.join(workspacePath, cluster.folder))
      }

      // Apply the new content of the manifest
      await FS.writeFileSync(Path.join(workspacePath, 'connections.json'), applyJSONBeautify(clustersManifest))
      ...
      // Return failure
      if (parseInt(`${e}`) != 0) {
        log.warning('Something went wrong with an imported cluster ', {'error': e})
        return 0
      }
    }
    ...
    log.warning('Failed to save workspace', {'workspace': workspace.name, 'error': e})
...
let updateWorkspace = async (workspace) => {
  ...
  // Add log about this process
  log.info('Updating workspace...', {'workspace': workspace.name})
  ...
  try {
    ...
    try {
      ...
      log.warning('Failed to parse workspaces', e)
      workspaces = []
    }
    ...
    try {
      ...
      log.warning('Failed to move workspace', {'workspace': workspace.name, 'old_path': originalPath, 'new_path': newPath, 'error': e})
    }
    ...
    log.warning('Somrthing went wrong updating workspace', {'workspace': workspace.name, 'error': e})
  } finally {
...
let deleteWorkspace = async (workspace, workspaces, keepFiles = false) => {
  ...
  // Add log about this process
  log.info('Deleting workspace...', {'workspace': workspace.name, 'keepFiles': keepFiles})
  ...
  try {
    ...
    try {
      if (!keepFiles) { // Remove the entire workspace folder; including all sub files and folders and make sure to force that process
        await FS.rmSync(workspaceFolderPath, {
          recursive: true,
          force: true
        })
      } else { // Keep the workspace's folder, however, add a prefix `_DEL_` with random digits
        await FS.moveSync(workspaceFolderPath, `${workspaceFolderPath}_DEL_${getRandomID(5)}`, {
          overwrite: true
        })
      }
    } catch (e) {
      log.error('Something went wrong removing workspace data files', {'workspace': workspace.name, 'keepFiles': keepFiles, 'error': e})
    ...
    log.warning('Something went wrong removing workspace', {'workspace': workspace.name, 'keepFiles': keepFiles, 'error': e})
  } finally {
