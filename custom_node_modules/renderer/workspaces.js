...
let getWorkspaces = async () => {
  ...
  const workspacesHome = Path.join(await getUserDataDir(), 'workspaces')
  const workspaceConfigFile = Path.join(workspacesHome, 'workspaces.json')
  ...
  try {
    // Get all workspaces
    let savedWorkspaces = await FS.readFileSync(workspaceConfigFile, 'utf8')
    ...
    for (let workspace of workspaces) {
      try {
        ...
        let folderPath = !workspace.defaultPath ? workspace.path : Path.join(UserDataDir, 'workspaces'),
...
let saveWorkspace = async (workspace) => {
  ...
  try {
    ...
    console.log(`Saving workspace ${JSON.stringify(workspace)}: ${e}`)
  }
  ...
  try {
    ...
    let workspaces = []
    try {
      workspaces = await FS.readFileSync(workspaceConfigFile, 'utf8')
    } catch(e) {
      addLog(`Cannot read ${workspaceConfigFile}`, 'error')
    }
    ...
    let workspacePath = Path.join(workspacesHome, folder)
    ...
    console.log(`Creating ${workspacePath}`)
    await FS.mkdirSync(workspacePath, { recursive: true })
    ...
    await FS.writeFileSync(workspaceConfigFile, applyJSONBeautify(workspaces))
...
let updateWorkspace = async (workspace) => {
  ...
  try {
    // Get all workspaces
    let workspaces = await FS.readFileSync(workspaceConfigFile, 'utf8')
    ...
    let defaultPath = workspacesHome,
...
let deleteWorkspace = async (workspace, workspaces) => {
  ...
  try {
    ...
    let folderPath = !workspace.defaultPath ? workspace.path : workspacesHome,
